// create an enum for moving direction
const Direction = {
  RIGHT: 'RIGHT',
  LEFT: 'LEFT',
  UP: 'UP',
...
class PlayerContainer extends Phaser.GameObjects.Container {
  constructor(scene, x, y, key, frame, health, maxHealth, id, attackAudio) {  
    ...
    this.attackAudio = attackAudio; // audio for atatcking
  ...
  update(cursors) {
    ...
    if (cursors.left.isDown) {
      ...
      this.player.flipX = false;    // do not flip the player 
    } else if (cursors.right.isDown) {
      this.body.setVelocityX(this.velocity);
      this.currentDirection = Direction.RIGHT; 
      this.weapon.setPosition(40, 0); 
      this.player.flipX = true;    // flip the player 
    }
